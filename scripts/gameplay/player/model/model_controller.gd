...
func rotate_towards_with_interpolation(delta : float, direction : Vector3, speed : float, _normal : Vector3, ) -> void:
...
func rotate_towards(direction : Vector3, _normal : Vector3) -> void:
	var target_pos = position - direction
	transform = transform.looking_at(target_pos, _normal).orthonormalized()
# 